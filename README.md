...
Конфигурация из `docker-compose.yml` предназначена для локальной разработки:
используются Mailpit, dev-пароли и Flask debug mode. Для production задайте
сильный `SECRET_KEY`, выключите `FLASK_DEBUG`, используйте внешний Postgres/SMTP
и запускайте приложение через WSGI-сервер.
...
Исторические прототипы, локальные базы данных, выгрузки опросов и загруженные
пользовательские файлы не хранятся в Git.
