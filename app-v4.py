...
SMTP_CONFIG = {
    ...
    'smtp_port': 587,
    'smtp_username': os.getenv('SMTP_USERNAME', 'your-email@gmail.com'),  # Используем переменные окружения
    'smtp_password': os.getenv('SMTP_PASSWORD', 'your-app-password')  # Используем переменные окружения
}

# Добавляем проверку существования директории для загрузки файлов
...
def get_db_connection():
    try:
        ...
        # Создаем таблицу random_coffee_meetings, если она не существует
        cur.execute('''
        ...
        # Создаем таблицу skills, если она не существует
        cur.execute('''
        ...
        # Создаем таблицу user_skills для связи пользователей и навыков
        cur.execute('''
        ...
        # Создаем таблицу blog_posts, если она не существует
        cur.execute('''
        ...
        # Добавляем базовые навыки, если таблица пуста
        ...
        if cur.fetchone()[0] == 0:
            ...
            cur.executemany('INSERT INTO skills (name, category) VALUES (%s, %s)', basic_skills)
        ...
        try:
            ...
            if teach_skills:
                cur.execute('''
                    ...
                    FROM skills
                    WHERE name = ANY(%s)
                ''', (user_id, teach_skills))
            
            # Добавляем навыки для изучения
...
def login():
    if request.method == 'GET':
        # Очищаем все flash-сообщения при входе на страницу
        session.clear()
    if request.method == 'POST':
...
def update_profile(user_id):
    ...
    try:
        ...
        try:
            ...
            # Добавляем новые навыки для обучения
            teach_skills = request.form.getlist('teach_skills')
            if teach_skills:
                cur.execute('''
                    INSERT INTO user_skills (user_id, skill_id, skill_type)
            ...
            # Добавляем новые навыки для изучения
            learn_skills = request.form.getlist('learn_skills')
            if learn_skills:
                cur.execute('''
                    INSERT INTO user_skills (user_id, skill_id, skill_type)
...
def lk(user_id):
    ...
    cur.execute('''
        SELECT u.id, u.first_name, u.last_name, u.birth_date, u.location, 
               u.teach_skills, u.learn_skills, u.photo_path, u.about,
               u.work_place, u.study_place, u.social_vk, u.social_tg, u.social_gh,
               COUNT(DISTINCT r.id) as review_count,
               AVG(r.rating) as avg_rating
        FROM users u
    ...
    all_skills = [
        ...
        'Git', 'Linux', 'Agile', 'Scrum', 'UI/UX Design', 'Data Science', 'Machine Learning'
    ]
    ...
    user_data = {
        ...
        'birth_date': user[3],
        'location': user[4],
        'teach_skills': user[5] if user[5] else [],
        'learn_skills': user[6] if user[6] else [],
        'photo_path': user[7],
        'about': user[8],
        'work_place': user[9],
        'study_place': user[10],
        ...
        'social_tg': user[12],
        'social_gh': user[13],
        'rating': float(user[15]) if user[15] else 0,
        'review_count': user[14] or 0,
        'stats': {
...
def join_challenge(challenge_type):
    if 'user_id' not in session:
        flash('Для участия в челлендже необходимо войти в систему', 'error')
        return redirect(url_for('login'))
...
def join_random_coffee():
    if request.method == 'POST':
        if 'user_id' not in session:
            flash('Для создания встречи необходимо войти в систему', 'error')
            return redirect(url_for('login'))
        ...
        try:
            ...
            if not is_online and not city:
                flash('Для оффлайн-встречи необходимо указать город', 'error')
                return redirect(url_for('join_random_coffee'))
            ...
            flash(f'Ошибка при создании встречи: {str(e)}', 'error')
            if 'conn' in locals():
                conn.rollback()
...
def join_mastermind():
    if request.method == 'POST':
        if 'user_id' not in session:
            flash('Для участия в мастермайнде необходимо войти в систему', 'error')
            return redirect(url_for('login'))
...
def networking_moscow():
    if request.method == 'POST':
        if 'user_id' not in session:
            flash('Для регистрации на мероприятие необходимо войти в систему', 'error')
            return redirect(url_for('login'))
...
def publish_article():
    try:
        ...
        if not article_url.startswith('https://telegra.ph/'):
            flash('Ссылка должна вести на telegra.ph', 'error')
            return redirect(url_for('blog'))
        ...
        flash(f'Ошибка при публикации статьи: {str(e)}', 'error')
        if 'conn' in locals():
            conn.rollback()
